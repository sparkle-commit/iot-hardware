# TEMPLATE - generated from KPG-Gondrong config, adjust software id / serial number to the actual device before importing
# model = wAPR-2nD  (adjust if a different unit is used for Wonosari)
...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=jogjaNMS123 wpa2-pre-shared-key=jogjaNMS123 \
    supplicant-identity=""
/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=NMS-JOGJA
/iot lora servers
...
add name=dhcp_pool1 ranges=192.168.58.4-192.168.58.14
/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1
/system script
add dont-require-permissions=no name=wonosari-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":\n:local snmpPort 161\n:local community \"public\"\n:local broker \"mqtt-broker\"\n\n:local switches {\"192.168.58.1\"}\n\n:local sfpMap {\n    \"192.168.58.1\"={\n        \"3\"={topic=\"NMS/WONOSARIIM3/SWITCH-1/SFP-1\"; ip=\"192.168.252.25\"};\n        \"4\"={topic=\"NMS/WONOSARIIM3/SWITCH-1/SFP-2\"; ip=\"192.168.252.29\"}\n    }\n}\n\n:local sysTopicMap {\n    \"192.168.58.1\"=\"NMS/WONOSARIIM3/SWITCH-1/SYSTEM\"\n}\n\n:local tsMs ([:tonsec [:timestamp]] / 1000000)\
    \n\
...
# NOTE: mac-address di bawah masih placeholder (disalin polanya dari Gondrong FE:0F:4A:9F:99:97) - GANTI biar tidak bentrok antar gateway
/interface ovpn-server server
add mac-address=FE:0F:4A:9F:99:98 name=ovpn-server1
/ip address
add address=192.168.58.2 interface=lo network=192.168.58.2
add address=192.168.58.2/28 interface=ether1 network=192.168.58.0
/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.58.0/28 gateway=192.168.58.2
/ip dns
set servers=8.8.8.8
/ip firewall nat
...
/ip route
add disabled=no dst-address=192.168.252.24/30 gateway=192.168.58.1 \
    routing-table=main
add disabled=no dst-address=192.168.252.28/30 gateway=192.168.58.1 \
    routing-table=main
add disabled=no dst-address=192.168.58.16/28 gateway=192.168.58.1 \
    routing-table=main
add disabled=no dst-address=192.168.58.32/28 gateway=192.168.58.1 \
    routing-table=main
/ipv6 nd
...
/system scheduler
add interval=5m name=wonosari on-event=\
    "/system script run wonosari-publish-v1" policy=\
