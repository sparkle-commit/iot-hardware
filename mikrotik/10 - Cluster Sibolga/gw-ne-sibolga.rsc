...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=sibolgaNMS123 wpa2-pre-shared-key=sibolgaNMS123 \
    supplicant-identity=""

/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=nms-sibolga
...
add name=dhcp_pool1 ranges=192.168.67.4-192.168.67.14

/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1

/system script
add dont-require-permissions=no name=sibolga-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":
:local snmpPort 161
:local community \"public\"
:local broker \"mqtt-broker\"

:local switches {\"192.168.67.1\"}

:local sfpMap {
    \"192.168.67.1\"={
        \"1\"={topic=\"NMS/SIBOLGA_MT/SWITCH-1/SFP-1\"; ip=\"192.168.252.81\"}
    }
}

:local sysTopicMap {
    \"192.168.67.1\"=\"NMS/SIBOLGA_MT/SWITCH-1/SYSTEM\"
}
...
/ip address
add address=192.168.67.2 interface=lo network=192.168.67.2
add address=192.168.67.2/28 interface=ether1 network=192.168.67.0

/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.67.0/28 gateway=192.168.67.2
...
/ip route
add disabled=no dst-address=192.168.252.80/30 gateway=192.168.67.1 \
    routing-table=main
add disabled=no dst-address=192.168.67.16/28 gateway=192.168.67.1 \
    routing-table=main
...
/system scheduler
add interval=5m name=sibolga on-event=\
    "/system script run sibolga-publish-v1" policy=\
