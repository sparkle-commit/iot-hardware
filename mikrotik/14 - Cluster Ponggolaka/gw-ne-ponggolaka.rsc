...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=ponggolakaNMS123 wpa2-pre-shared-key=ponggolakaNMS123 \
    supplicant-identity=""

/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=nms-ponggolaka
...
add name=dhcp_pool1 ranges=192.168.71.4-192.168.71.14

/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1

/system script
add dont-require-permissions=no name=ponggolaka-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":
:local snmpPort 161
:local community \"public\"
:local broker \"mqtt-broker\"

:local switches {\"192.168.71.1\"}

:local sfpMap {
    \"192.168.71.1\"={
        \"1\"={topic=\"NMS/PONGGOLAKA/SWITCH-1/SFP-1\"; ip=\"192.168.252.105\"};
        \"2\"={topic=\"NMS/PONGGOLAKA/SWITCH-1/SFP-2\"; ip=\"192.168.252.109\"};
        \"3\"={topic=\"NMS/PONGGOLAKA/SWITCH-1/SFP-3\"; ip=\"192.168.252.113\"};
        \"4\"={topic=\"NMS/PONGGOLAKA/SWITCH-1/SFP-4\"; ip=\"192.168.252.117\"}
    }
}

:local sysTopicMap {
    \"192.168.71.1\"=\"NMS/PONGGOLAKA/SWITCH-1/SYSTEM\"
}
...
/ip address
add address=192.168.71.2 interface=lo network=192.168.71.2
add address=192.168.71.2/28 interface=ether1 network=192.168.71.0

/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.71.0/28 gateway=192.168.71.2
...
/ip route
add disabled=no dst-address=192.168.252.104/30 gateway=192.168.71.1 routing-table=main
add disabled=no dst-address=192.168.252.108/30 gateway=192.168.71.1 routing-table=main
add disabled=no dst-address=192.168.252.112/30 gateway=192.168.71.1 routing-table=main
add disabled=no dst-address=192.168.252.116/30 gateway=192.168.71.1 routing-table=main
add disabled=no dst-address=192.168.71.16/28 gateway=192.168.71.1 routing-table=main
add disabled=no dst-address=192.168.71.32/28 gateway=192.168.71.1 routing-table=main
add disabled=no dst-address=192.168.71.48/28 gateway=192.168.71.1 routing-table=main
add disabled=no dst-address=192.168.71.64/28 gateway=192.168.71.1 routing-table=main
...
/system scheduler
add interval=5m name=ponggolaka on-event=\
    "/system script run ponggolaka-publish-v1" policy=\
