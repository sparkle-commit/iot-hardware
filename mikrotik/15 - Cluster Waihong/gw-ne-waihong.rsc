...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=waihongNMS123 wpa2-pre-shared-key=waihongNMS123 \
    supplicant-identity=""

/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=nms-waihongep
...
add name=dhcp_pool1 ranges=192.168.72.4-192.168.72.14

/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1

/system script
add dont-require-permissions=no name=waihongep-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":
:local snmpPort 161
:local community \"public\"
:local broker \"mqtt-broker\"

:local switches {\"192.168.72.1\"}

:local sfpMap {
    \"192.168.72.1\"={
        \"1\"={topic=\"NMS/WAIHONG_EP/SWITCH-1/SFP-1\"; ip=\"192.168.252.121\"};
        \"2\"={topic=\"NMS/WAIHONG_EP/SWITCH-1/SFP-2\"; ip=\"192.168.252.125\"}
    }
}

:local sysTopicMap {
    \"192.168.72.1\"=\"NMS/WAIHONG_EP/SWITCH-1/SYSTEM\"
}
...
/ip address
add address=192.168.72.2 interface=lo network=192.168.72.2
add address=192.168.72.2/28 interface=ether1 network=192.168.72.0

/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.72.0/28 gateway=192.168.72.2
...
/ip route
add disabled=no dst-address=192.168.252.120/30 gateway=192.168.72.1 routing-table=main
add disabled=no dst-address=192.168.252.124/30 gateway=192.168.72.1 routing-table=main
add disabled=no dst-address=192.168.72.16/28 gateway=192.168.72.1 routing-table=main
add disabled=no dst-address=192.168.73.16/28 gateway=192.168.72.1 routing-table=main
...
/system scheduler
add interval=5m name=waihongep on-event=\
    "/system script run waihongep-publish-v1" policy=\
