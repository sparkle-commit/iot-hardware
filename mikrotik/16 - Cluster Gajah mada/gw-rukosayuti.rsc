...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=rukosayutiNMS123 wpa2-pre-shared-key=rukosayutiNMS123 \
    supplicant-identity=""

/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=nms-rukosayuti
...
add name=dhcp_pool1 ranges=192.168.57.4-192.168.57.14

/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1

/system script
add dont-require-permissions=no name=rukosayuti-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":
:local snmpPort 161
:local community \"public\"
:local broker \"mqtt-broker\"

:local switches {\"192.168.57.1\"}

:local sfpMap {
    \"192.168.57.1\"={
        \"1\"={topic=\"NMS/RUKO_SAYUTI/SWITCH-1/SFP-1\"; ip=\"192.168.252.2\"};
        \"2\"={topic=\"NMS/RUKO_SAYUTI/SWITCH-1/SFP-2\"; ip=\"192.168.252.6\"};
        \"3\"={topic=\"NMS/RUKO_SAYUTI/SWITCH-1/SFP-3\"; ip=\"192.168.252.10\"}
    }
}

:local sysTopicMap {
    \"192.168.57.1\"=\"NMS/RUKO_SAYUTI/SWITCH-1/SYSTEM\"
}
...
/ip address
add address=192.168.57.2 interface=lo network=192.168.57.2
add address=192.168.57.2/28 interface=ether1 network=192.168.57.0

/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.57.0/28 gateway=192.168.57.2
...
/ip route
add disabled=no dst-address=192.168.252.0/30 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.252.4/30 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.252.8/30 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.252.12/30 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.252.16/30 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.252.20/30 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.57.16/28 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.57.32/28 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.57.48/28 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.57.64/28 gateway=192.168.57.1 routing-table=main
add disabled=no dst-address=192.168.57.72/28 gateway=192.168.57.1 routing-table=main
...
/system scheduler
add interval=5m name=rukosayuti on-event=\
    "/system script run rukosayuti-publish-v1" policy=\
