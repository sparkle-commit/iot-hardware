...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=stabatNMS123 wpa2-pre-shared-key=stabatNMS123 \
    supplicant-identity=""
/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=nms-stabat
/iot lora servers
...
add name=dhcp_pool1 ranges=192.168.60.4-192.168.60.14
/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1
/system script
add dont-require-permissions=no name=stabat-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":\n:local snmpPort 161\n:local community \"public\"\n:local broker \"mqtt-broker\"\n\n:local switches {\"192.168.60.1\"}\n\n:local sfpMap {\n    \"192.168.60.1\"={\n        \"3\"={topic=\"NMS/STABAT_TB/SWITCH-1/SFP-1\"; ip=\"192.168.252.37\"};\n        \"4\"={topic=\"NMS/STABAT_TB/SWITCH-1/SFP-2\"; ip=\"192.168.252.41\"}\n    }\n}\n\n:local sysTopicMap {\n    \"192.168.60.1\"=\"NMS/STABAT_TB/SWITCH-1/SYSTEM\"\n}\n\n:local tsMs ([:tonsec [:timestamp]] / 1000000)\
    \n\
...
/ip address
add address=192.168.60.2 interface=lo network=192.168.60.2
add address=192.168.60.2/28 interface=ether1 network=192.168.60.0
/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.60.0/28 gateway=192.168.60.2
/ip dns
set servers=8.8.8.8
/ip firewall nat
...
/ip route
add disabled=no dst-address=192.168.252.36/30 gateway=192.168.60.1 \
    routing-table=main
add disabled=no dst-address=192.168.252.40/30 gateway=192.168.60.1 \
    routing-table=main
add disabled=no dst-address=192.168.60.16/28 gateway=192.168.60.1 \
    routing-table=main
add disabled=no dst-address=192.168.60.32/28 gateway=192.168.60.1 \
    routing-table=main
/ipv6 nd
...
add interval=5m name=wonosari on-event=\
    "/system script run stabat-publish-v1" policy=\
