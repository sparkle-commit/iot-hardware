...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=kualasimpangNMS123 wpa2-pre-shared-key=kualasimpangNMS123 \
    supplicant-identity=""
/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=nms-kualasimpang
/iot lora servers
...
add name=dhcp_pool1 ranges=192.168.61.4-192.168.61.14
/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1
/system script
add dont-require-permissions=no name=kualasimpang-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":\n:local snmpPort 161\n:local community \"public\"\n:local broker \"mqtt-broker\"\n\n:local switches {\"192.168.61.1\"}\n\n:local sfpMap {\n    \"192.168.61.1\"={\n        \"3\"={topic=\"NMS/KUALA_SIMPANG_NE/SWITCH-1/SFP-1\"; ip=\"192.168.252.45\"}\n    }\n}\n\n:local sysTopicMap {\n    \"192.168.61.1\"=\"NMS/KUALA_SIMPANG_NE/SWITCH-1/SYSTEM\"\n}\n\n:local tsMs ([:tonsec [:timestamp]] / 1000000)\
    \n\
...
/ip address
add address=192.168.61.2 interface=lo network=192.168.61.2
add address=192.168.61.2/28 interface=ether1 network=192.168.61.0
/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.61.0/28 gateway=192.168.61.2
/ip dns
set servers=8.8.8.8
/ip firewall nat
...
/ip route
add disabled=no dst-address=192.168.252.44/30 gateway=192.168.61.1 \
    routing-table=main
add disabled=no dst-address=192.168.61.16/28 gateway=192.168.61.1 \
    routing-table=main
/ipv6 nd
...
add interval=5m name=rataagung on-event=\
    "/system script run kualasimpang-publish-v1" policy=\
