...
add authentication-types=wpa-psk,wpa2-psk mode=dynamic-keys name=profile1 \
    wpa-pre-shared-key=langsaNMS123 wpa2-pre-shared-key=langsaNMS123 \
    supplicant-identity=""
/interface wireless
set [ find default-name=wlan1 ] disabled=no mode=ap-bridge security-profile=\
    profile1 ssid=nms-langsa
/iot lora servers
...
add name=dhcp_pool1 ranges=192.168.66.4-192.168.66.14
/ip dhcp-server
add address-pool=dhcp_pool1 interface=bridge1 name=dhcp1
/system script
add dont-require-permissions=no name=langsa-publish-v1 owner=admin policy=ftp,reboot,read,write,policy,test,password,sniff,sensitive,romon source=":
:local snmpPort 161
:local community \"public\"
:local broker \"mqtt-broker\"

:local switches {\"192.168.66.1\"}

:local sfpMap {
    \"192.168.66.1\"={
        \"3\"={topic=\"NMS/LANGSA_MT/SWITCH-1/SFP-1\"; ip=\"192.168.252.73\"};
        \"4\"={topic=\"NMS/LANGSA_MT/SWITCH-1/SFP-2\"; ip=\"192.168.252.77\"}
    }
}

:local sysTopicMap {
    \"192.168.66.1\"=\"NMS/LANGSA_MT/SWITCH-1/SYSTEM\"
}
...
/ip address
add address=192.168.66.2 interface=lo network=192.168.66.2
add address=192.168.66.2/28 interface=ether1 network=192.168.66.0
/ip dhcp-server network
add gateway=0.0.0.1
add address=192.168.66.0/28 gateway=192.168.66.2
/ip dns
set servers=8.8.8.8
/ip firewall nat
...
/ip route
add disabled=no dst-address=192.168.252.72/30 gateway=192.168.66.1 \
    routing-table=main
add disabled=no dst-address=192.168.252.76/30 gateway=192.168.66.1 \
    routing-table=main
add disabled=no dst-address=192.168.66.16/28 gateway=192.168.66.1 \
    routing-table=main
add disabled=no dst-address=192.168.66.32/28 gateway=192.168.66.1 \
    routing-table=main
/ipv6 nd
...
/system scheduler
add interval=5m name=langsa on-event=\
    "/system script run langsa-publish-v1" policy=\
